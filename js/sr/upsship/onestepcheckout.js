...
function save_shipping_method(shipping_method_url, update_shipping_payment, update_shipping_review, notValidateForm) {
    ...
    var parameters = {
        ...
        form_key: form_key,
        shipping_ups_pickup_id: $$("input[name=shipping_ups_pickup_id]")[0].value,
        shipping_additional_information: $$("input[name=shipping_additional_information]")[0].value
    };
    ...
    var request = new Ajax.Request(shipping_method_url, {
        ...
        onSuccess: function (transport) {
            if (transport.status == 200) {
                ...
                checkvalidEmail(notValidateForm);
                if (window.UPSShip && window.UPSShip.isActiveMethod()) {
                    if (!parameters['shipping_ups_pickup_id']) {
                        disablePlaceOrderButton();
                    }
                }
            }
